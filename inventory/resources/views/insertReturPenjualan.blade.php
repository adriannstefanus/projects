...
@section('content')
    <div class="px-5">
        <form action="/returPenjualan/form/submit" onsubmit="cekBeforeRetur(event)" method="POST">
            @csrf
            <div class="row">
                <div class="form-group col input-group-sm">
                    <label>Nama Pelanggan</label>
                    <input readonly type="text" class="form-control" id="pelanggan" name="pelanggan"
                        value="{{ $faktur->trpenawaran->customer->Nama}}">
                </div>
                <div class="form-group col input-group-sm">
                ...
                <div class="form-group col input-group-sm">
                    ...
                    <input readonly type="text" class="form-control" id="tglFaktur" name="tglFaktur"
                        value="{{ date_format(date_create($faktur->TglFaktur), 'd M Y')}}">
                </div>
            </div>
            ...
            <div class="row">
                <div class="form-group col input-group-sm">
                    <label>No SO</label>
                    <input readonly type="text" class="form-control" id="noSo" name="noSo"
                        value="{{ $faktur->trpenawaran->NoSO}}">
                </div>
                <div class="form-group col input-group-sm">
                    <label>Tgl SO</label>
                    <input readonly type="text" class="form-control" id="tglSo" name="tglSo"
                        value="{{ date_format(date_create($faktur->trpenawaran->TglSO), 'd M Y')}}">
                </div>
                <div class="form-group col input-group-sm">
                    <label>Tgl Jatuh Tempo</label>
                    <input readonly type="text" class="form-control" id="tglJatuhTempo" name="tglJatuhTempo"
                        value="{{ date_format(date_create($faktur->trpenawaran->TglJatuhTempoSO), 'd M Y')}}">
                </div>
            </div>
            ...
            <table id="trans-table" class="table table-striped table-hover" border="1px solid #343e42">
                ...
                <tbody>
                    @foreach($faktur->trpenawaran->barangs as $x)
                        <tr>
